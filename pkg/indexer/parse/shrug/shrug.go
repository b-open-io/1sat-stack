...
func (i *ShrugIndexer) Parse(idxCtx *indexer.IndexContext, vout uint32) any {
	...
	if shrug != nil {
		if shrug.Id == nil {
			shrug.Status = Valid
			id := types.NewOutpointFromHash(idxCtx.Txid, vout)
			output.AddEvent(SHRUG_TAG + ":deploy")
			output.AddEvent(SHRUG_TAG + ":" + shrug.Status.String() + ":" + id.OrdinalString())
			if shrug.Amount.Cmp(interpreter.Zero) == 0 {
				output.AddEvent(SHRUG_TAG + ":mint:" + id.OrdinalString())
			}
		} else {
			output.AddEvent(SHRUG_TAG + ":" + shrug.Status.String() + ":" + shrug.Id.OrdinalString())
		}
		return shrug
	}
...
func parseScript(s *script.Script) *Shrug {
	...
	if op, err := s.ReadOp(&pos); err != nil {
		...
		shrug.Id = types.NewOutpointFromBytes(op.Data)
	} else if op, err = s.ReadOp(&pos); err != nil {
		return nil
...
func (i *ShrugIndexer) PreSave(idxCtx *indexer.IndexContext) {
	...
	for tokenId, token := range tokens {
		...
		for j, output := range token.outputs {
			...
			if shrug.Amount.Cmp(interpreter.Zero) == 0 && token.status == Valid {
				output.AddEvent(SHRUG_TAG + ":mint:" + shrug.Id.OrdinalString())
			}
		}
	}
}
