...
func (i *Bsv21Indexer) Parse(idxCtx *indexer.IndexContext, vout uint32) any {
	...
	case "deploy+mint":
		...
		if add, err := script.NewAddressFromPublicKey(pubKey, idxCtx.Network == types.Mainnet); err == nil {
	...
	case "transfer", "burn":
		if id, ok := insc.JsonMap["id"]; !ok {
			return nil
		} else if _, err = types.NewOutpointFromString(id); err != nil {
			return nil
		} else {
			bsv21.Id = id
		}
...
func (i *Bsv21Indexer) PreSave(idxCtx *indexer.IndexContext) {
	ctx := bsv21Ctx{
		tokens: map[string]*bsv21Token{},
	}
			...
			if bsv21, ok := bsv21Data.(*Bsv21); ok {
				if bsv21.Status == Pending {
					...
					if token, ok := ctx.tokens[bsv21.Id]; ok {
						...
						token.token = bsv21
						token.deps = append(token.deps, indexer.OutpointToTypes(&spend.Outpoint))
					}
				}
			}
		}
