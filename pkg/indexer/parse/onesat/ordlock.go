...
func (i *OrdLockIndexer) Parse(idxCtx *indexer.IndexContext, vout uint32) any {
	...
	if sCryptPrefixIndex := bytes.Index(*scr, OrdLockPrefix); sCryptPrefixIndex == -1 {
		...
		pkhash := types.PKHashFromBytes(ordLockOps[0].Data)
		...
		if bsv21Data, ok := output.Data[BSV21_TAG]; ok {
			...
			if origin.Outpoint != nil {
				ordLock.Tags = append(ordLock.Tags, origin.Outpoint.OrdinalString())
...
func (i *OrdLockIndexer) PreSave(idxCtx *indexer.IndexContext) {
