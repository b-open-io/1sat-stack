...
type Inscription struct {
	...
	File    *types.File       `json:"file,omitempty"`
...
func (i *InscriptionIndexer) Parse(idxCtx *indexer.IndexContext, vout uint32) any {
...
func (i *InscriptionIndexer) PreSave(idxCtx *indexer.IndexContext) {
...
func ParseInscription(output *txo.IndexedOutput, scr *script.Script, fromPos *int, bitcomData any) *Inscription {
	insc := &Inscription{
		File: &types.File{},
	}
	pos := *fromPos
...
ordLoop:
	for {
		...
		case 3:
			insc.Parent = types.NewOutpointFromBytes(op2.Data)
		case 9:
	...
	*fromPos = pos
	...
	if len(*scr) >= pos+25 && script.NewFromBytes((*scr)[pos:pos+25]).IsP2PKH() {
		pkhash := types.PKHashFromBytes((*scr)[pos+3 : pos+23])
		...
		pkhash := types.PKHashFromBytes((*scr)[pos+4 : pos+24])
		output.AddOwnerFromAddress(pkhash.Address())
	}
