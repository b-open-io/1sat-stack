...
func NewSubscriber(cfg *SubscriberConfig, q queue.Queue, cs config.Store, ct chaintracks.Chaintracks, jbClient *junglebus.Client, logger *slog.Logger) (*Subscriber, error) {
	if logger == nil {
		logger = slog.Default()
	}
...
func (s *Subscriber) Start(ctx context.Context) error {
	...
	// Batch management
	var batchMembers []queue.ScoredItem
	...
	flushBatch := func() error {
		...
		if err := s.queue.Enqueue(ctx, queueKey, batchMembers...); err != nil {
	...
	handler := junglebus.EventHandler{
		OnTransaction: func(txn *models.TransactionResponse) {
			...
			batchMembers = append(batchMembers, queue.ScoredItem{
				Member: txid[:],
				Score:  score,
			})
	...
	if s.config.EnableMempool {
		handler.OnMempool = func(txn *models.TransactionResponse) {
			...
			if err := s.queue.Enqueue(ctx, queueKey, queue.ScoredItem{
				Member: txid[:],
				Score:  score,
			}); err != nil {
