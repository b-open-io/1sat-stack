...
type BAPStore interface {
	...
	SaveAttestation(ctx context.Context, urnHash, bapID string, signer *Signer) error
	RevokeAttestation(ctx context.Context, urnHash, bapID string) error
...
CREATE TABLE IF NOT EXISTS bap_attestations (
	...
	PRIMARY KEY (topic_id, urn_hash, bap_id)
);
