...
// Routes handles the admin control-plane HTTP routes: setup wizard, config
// CRUD, auth admin, log viewer, and restart. Cross-service admin capabilities
// are mounted by the composition layer from their owning services.
type Routes struct {
...
func NewRoutes(cs config.Store, requestRestart func(), cfg *RoutesConfig, logger *slog.Logger, logStore *logging.SQLiteHandler) *Routes {
	return &Routes{
		configStore:    cs,
...
func (r *Routes) Register(guardedGroup fiber.Router, publicGroup fiber.Router, authHandler fiber.Handler) {
	...
	// API routes (require auth via AdminGuard on guardedGroup). Cross-service
	// admin routes (data browser, whitelist/blacklist, workers, topics,
	// lookups, opns crawl) are mounted by the composition layer from their
	// owning services at these same paths.
