...
| `tm_1sat_collection` | `DiscoveryTopicManager` (collection) | Collection root mints (MAP + SIGMA) |
| `tm_col_{collectionId}` | `MemberTopicManager` (collection) | Collection item mints for one collection |
