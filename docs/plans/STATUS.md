...
| [Microservice Split Implementation](./microservice-split-implementation.md) | **In Progress** | M1–M5 all implemented and gate-verified on branch `microservice-split`: pkg/node+modes, Queue, event-driven lifecycle, remote providers, gateway + per-service admin routes. mode:all parity holds; three-process (index/opns/gateway) smoke passes. Pending final whole-branch review before merge |
