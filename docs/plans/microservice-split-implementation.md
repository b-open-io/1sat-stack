...
- [ ] **Step 1:** In `pkg/node` wiring, gate txo OutputStore, indexer (IngestCtx/IngestSync), and PendingAuditor construction on `c.runsService("index")` — overlay-only processes never construct them, so they carry no txo. The overlay adapter's `IngestTx` stays **nil in all modes** (its state after the M1 parity fix): overlays never ingest into txo directly. Ingestion always flows through the broadcast→arcade→arc-status feedback loop, so `mode: all` behaves identically to a split deployment instead of taking an in-process shortcut a distributed deployment couldn't. **Do NOT reintroduce the Task 1.3 IngestTx closure** — that would double-ingest in `mode: all` (both the adapter and `handleAccepted` call `indexer.IngestTx`). NOTE (split-mode routing, connects to the SSE/GASP feed / gap 7): how the index service learns about a tx submitted to a *standalone* overlay process — overlay's Broadcaster → index `/tx`, vs the overlay broadcasting to arcade directly with the index picking it up via its own arc/GASP feed — is a wiring detail to confirm before shipping split overlay deployments; it does not affect `mode: all`.
